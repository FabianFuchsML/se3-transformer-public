...
class SE3TransformerAutoEncode(nn.Module):
    """SE(3) equivariant GCN with attention and autoencoding"""
    def __init__(self, num_layers: int, atom_feature_size: int, 
                 num_channels: int, num_nlayers: int=1, num_degrees: int=4, 
                 edge_dim: int=4, div: float=4, pooling: str='avg', n_heads: int=1, **kwargs):
        super().__init__()
        ...
        self.fc1 = nn.Linear(self.fibers['out'].n_features, self.fibers['out'].n_features)
        self.fc2 = nn.Linear(self.fibers['out'].n_features, 30) # auto encode dimensions
        self.fc3 = nn.Linear(30, self.fibers['out'].n_features)
        self.fc4 = nn.Linear(self.fibers['out'].n_features, 1)
        print(self.Gblock)
